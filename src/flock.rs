...
impl Filesystem {
    ...
    pub fn create_dir(&self) -> anyhow::Result<()> {
        fs_err::create_dir_all(&self.root).map_err(anyhow::Error::from)
    }
    ...
    /// error.
    pub fn open_ro_shared<P>(&self, path: P, shell: &mut Shell, msg: &str) -> anyhow::Result<FileLock>
        where
            P: AsRef<Path>,
    {
...
fn try_acquire(path: &Path, lock_try: &dyn Fn() -> io::Result<()>) -> anyhow::Result<bool> {
